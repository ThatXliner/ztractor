...
async function main() {
	...
	if (!existsSync(translatorsDir)) {
		console.error(
			"❌ Translators directory not found. Run: git submodule update --init",
		);
		process.exit(1);
	}
	...
	for (const [type, count] of Object.entries(translatorsByType)) {
		const typeName =
			{ "1": "Import", "2": "Export", "4": "Web", "8": "Search" }[type] ||
			`Type ${type}`;
...
`;
	...
	registryCode +=
		"export const TRANSLATORS_REGISTRY: TranslatorRegistryEntry[] = [\n";
	...
	console.log(
		`   Registry file size: ${(registryCode.length / 1024 / 1024).toFixed(2)} MB`,
	);
	console.log(
		`   Average translator size: ${(totalCodeSize / translators.length / 1024).toFixed(2)} KB`,
	);
	console.log(
