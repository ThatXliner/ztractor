 ...
 */
let getTranslatorByIdFn:
	| ((id: string) => TranslatorRegistryEntry | null)
	| null = null;
 ...
 */
async function getTranslatorById(
	translatorId: string,
...
export async function executeDetectWeb(
    ...
    `;
		...
		console.error(
			`Error executing detectWeb for ${translator.metadata.label}:`,
			e,
		);
		return null;
	}
...
export async function executeDoWeb(
	...
	return new Promise((resolve, reject) => {
      ...
      `;
			...
			console.error(
				`Error executing doWeb for ${translator.metadata.label}:`,
				e,
			);
...
function createSandbox(
	...
	const items: Item[] = [];
	let selectItemsCallback: ((items: Record<string, string>) => void) | null =
		null;
